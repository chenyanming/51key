...
#define keyscan_mask 0x00//0：按下为低电平；0xff：按下为高电平
...
//只是用来存放输入引脚的状态而已，可以存放8个引脚的电平，这里一开始全部是低电平。
...
*/
	if ((keystu&1<<4)==1<<4)
	{
		...
		*/
		...
		/*
		** 如果6个输入引脚都是低电平，代表没有按键被按下（没按下是低电平）, 左移一个1，并保留keystu的低3位;
		** 如果有其中一个引脚是低电平，就左移一个0（用<<运算符）,并保留keystu的低3位。保留的同时也对扫描标志位清0
		*/
...
/*
**定时器计时部分
**虽然是2ms中断一次，但是，控制键盘扫描的位是100ms才开启一次的。而且，在主程序中没有严格控制2ms内必须完成一个主循环。因此在主程序中工作了20ms还是可以接受的。
*/
...
static void independent_key_check(void)
{
		...
		if ((keyscan&0xff)!=keyscan_mask) //只要有一个引脚不为0，就继续(不等于和恒等于是完全不同的!这里换成==0就不同结果了)
		{
